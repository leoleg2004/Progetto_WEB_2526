...
Il progetto è configurato di default per connettersi a MySQL con utente `root` e **nessuna password** (password vuota). Se l'installazione MySQL locale possiede credenziali differenti (es. password impostata):
1. Aprire il file `run.sh` (Mac/Linux) o `run.bat` (Windows), individuare la variabile `DB_USER` e aggiungere eventualmente una variabile `DB_PASS` o modificare la connessione.
