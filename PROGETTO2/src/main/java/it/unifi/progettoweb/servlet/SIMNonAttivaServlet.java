...
import it.unifi.progettoweb.dao.SIMNonAttivaDAO;
...
@WebServlet(urlPatterns = {"/sim_non_attive.php"})
public class SIMNonAttivaServlet extends HttpServlet {
    
    private SIMNonAttivaDAO dao;

    @Override
    public void init() throws ServletException {
        super.init();
        ThymeleafConfig.buildTemplateEngine(getServletContext());
        dao = new SIMNonAttivaDAO();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int totalRows = dao.countSimNonAttive(cercaCodice);
        int totalPages = (int) Math.ceil((double) totalRows / limit);

        List<SIMNonAttiva> sim = dao.findSimNonAttive(cercaCodice, limit, offset);
        ...
        templateEngine.process("sim_non_attive", ctx, response.getWriter());
    }
}
