...
import it.unifi.progettoweb.dao.SIMAttivaDAO;
...
public class SIMAttivaServlet extends HttpServlet {
    
    private SIMAttivaDAO simAttivaDAO;

    @Override
    public void init() throws ServletException {
        super.init();
        ThymeleafConfig.buildTemplateEngine(getServletContext());
        simAttivaDAO = new SIMAttivaDAO();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int totalRows = simAttivaDAO.countSimAttive(cercaCodice);
        int totalPages = (int) Math.ceil((double) totalRows / limit);

        List<SIMAttiva> simAttive = simAttivaDAO.findSimAttive(cercaCodice, limit, offset);
        ...
        ctx.setVariable("simAttive", simAttive);
        ...
        templateEngine.process("sim_attive", ctx, response.getWriter());
    }
}
